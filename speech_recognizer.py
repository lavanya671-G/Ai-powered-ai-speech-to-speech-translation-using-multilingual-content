...
if os.path.exists(FFMPEG_PATH):
    ...
    print("⚠️ Local FFmpeg not found, using system FFmpeg")
...
try:
    ...
    print("⚠️ Azure Speech SDK not installed — STT disabled.")
...
def split_audio_to_chunks(file_path, min_silence_len=1500, silence_thresh=-40, chunk_length_ms=CHUNK_LENGTH_MS):
    ...
    try:
        ...
        try:
            ...
            print(f"⚠️ Primary audio load failed: {e}, trying FFmpeg fallback...")
        ...
        if not chunks or len(chunks) == 0:
            print("⚠️ No silence detected, using fixed-length chunks")
        ...
        for i, chunk_path in enumerate(chunk_files):
            try:
                ...
                if not os.path.exists(chunk_path) or os.path.getsize(chunk_path) == 0:
                    print(f"⚠️ Chunk {i+1} is empty or missing, skipping")
                    continue
                ...
                if result.reason == speechsdk.ResultReason.RecognizedSpeech:
                    ...
                    if text and text.lower() != ".":  # Filter out empty or punctuation-only results
                        ...
                        print(f"⚠️ Chunk {i+1}: No meaningful speech detected")
                        
                elif result.reason == speechsdk.ResultReason.NoMatch:
                    print(f"⚠️ No speech detected in chunk {i+1}")
    ...
    def recognize_from_microphone(self):
        ...
        try:
            ...
            if result.reason == speechsdk.ResultReason.RecognizedSpeech:
                ...
                print("⚠️ No recognizable speech input.")
            elif result.reason == speechsdk.ResultReason.Canceled:
